    ...
    const fetchPrescriptions = async () => {
      try {
        const patientIdCookie = document.cookie
          ...
          .find(row => row.startsWith('patientId='))
          .split('=')[1];
        const response = await axios.get(`https://mrms-96547282c657.herokuapp.com/api/v1/prescription/all-patient-prescriptions?patientId=${patientIdCookie}`, {
          withCredentials: true
        });
    ...
    const fetchMedicalRecords = async () => {
      try {
        const patientIdCookie = document.cookie
          ...
          .find(row => row.startsWith('patientId='))
          .split('=')[1];
        const response = await axios.get(`https://mrms-96547282c657.herokuapp.com/api/v1/medical-record/all-patient-records?patientId=${patientIdCookie}`, {
          withCredentials: true
        });
