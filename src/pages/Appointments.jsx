...
function Appointments() {
  ...
  const handleCreateAppointment = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('https://mrms-96547282c657.herokuapp.com/api/v1/appointment/create', requestBody, {
      ...
      if (error.response && error.response.data ) {
        ...
        if (errorMessage === "Invalid appointment time") {
          ...
          const availableTimesResponse = await axios.get(`https://mrms-96547282c657.herokuapp.com/api/v1/appointment/get-user-available-times?userId=${formData.userid}&date=${formData.appDate}`);
    ...
    const fetchUsers = async () => {
      try {
        const patientIdCookie = document.cookie
          ...
          .find(row => row.startsWith('patientId='))
          .split('=')[1];
        const response = await axios.get(`https://mrms-96547282c657.herokuapp.com/api/v1/access-requests/all-patient-requests-approved?patientId=${patientIdCookie}`, {
          withCredentials: true
        });
    ...
    const fetchAppointments = async () => {
      try {
        const patientIdCookie = document.cookie
          ...
          .find(row => row.startsWith('patientId='))
          .split('=')[1];
        const response = await axios.get(`https://mrms-96547282c657.herokuapp.com/api/v1/appointment/all-patient-appointments?patientId=${patientIdCookie}`, {
          withCredentials: true
        });
