...
function HPAppointments() {
    ...
    const fetchAppointments = async () => {
      try {
        const userIdCookie = document.cookie
          ...
          .find(row => row.startsWith('userId='))
          .split('=')[1];
        const response = await axios.get(`https://mrms-96547282c657.herokuapp.com/api/v1/appointment/all-user-appointments?userId=${userIdCookie}`, {
          withCredentials: true
        });
  ...
  const updateAppointmentStatus = async (appointmentId, status, setAppointments) => {
    try {
      await axios.put(`https://mrms-96547282c657.herokuapp.com/api/v1/appointment/update-status?appointmentId=${appointmentId}&status=APPROVED`);
  ...
  const updateAppointment = async (appointmentId, visitType, comments) => {
    try {
      ...
      if (index !== -1) {
        ...
        await axios.put(`https://mrms-96547282c657.herokuapp.com/api/v1/appointment/update`, updatedAppointment);
  ...
  const deleteAppointment = async (appointmentId) => {
    try {
      await axios.delete(`hhttps://mrms-96547282c657.herokuapp.com/api/v1/appointment/delete?appointmentId=${appointmentId}`);
